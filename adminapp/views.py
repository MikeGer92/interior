...
def new_export_products_xls(request):
    response = HttpResponse(content_type='application/ms-excel')
    response['Content-Disposition'] = 'attachment; filename="products.xls"'

    wb = xlwt.Workbook(encoding='utf-8')
    ws = wb.add_sheet('all_roducts')
    ...
    columns = ['id', 'name', 'price', 'category_name' ]
    id = Product.objects.all().values_list('id')
    name = Product.objects.all().values_list('name')
    category_name = ProductCategory.objects.all().values_list('name')
    price = Product.objects.all().values_list('price')
    ...
    for col_num in range(len(columns)):
        # ws.write(row_num, col_num, columns[col_num], font_style)
        ws.write(row_num, col_num, row_data, font_style)

    # Sheet body, remaining rows
    # font_style = xlwt.XFStyle()
    # # cn = 'category'
    # rows = Product.objects.all().values_list('id', 'name', 'price' , 'ProductCategory__name')
    # for row in rows:
    #     row_num += 1
    #     for col_num in range(len(row)):
    #         ws.write(row_num, col_num, row[col_num], font_style)
